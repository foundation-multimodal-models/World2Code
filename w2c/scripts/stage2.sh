...
# Stage 2 (local open-source version): relation / region captions.
...
for (( chunk_index=0; chunk_index<chunk_num; chunk_index++ )); do
  CUDA_VISIBLE_DEVICES="${chunk_index}" \
    nohup python3 world2seq/relations.py \
      --config configs/relations.yaml \
      --model_path "${model_path}" \
      --chunk_index "${chunk_index}" --chunk_num "${chunk_num}" \
      --node_index "${node_index}" --node_num "${node_num}" \
      > "reservoir/output/relation_${chunk_index}_node_${node_index}.log" 2>&1 &
done
wait
python3 merge_results.py --config configs/relations.yaml
